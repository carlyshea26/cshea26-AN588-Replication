...
```{r}
paper_fig1 <- "~/Desktop/DATA/fig1paperversion.png"
paper_fig1 <- readPNG(paper_fig1) #reading my image in PNG format and converting it to an array/matrix
grid.raster(paper_fig1)
```
</details>
...
```{r}
paper_fig3 <- "~/Desktop/DATA/paperfig3.png"
paper_fig3 <- readPNG(paper_fig3) # reading my image in PNG format and converting it to an array/matrix
grid.raster(paper_fig3)
```
</details>
...
- this CSV contains all INIs measured from 1999 to 2020 and sorted by location, region and year
...
- basically the non-parametric version of the two-sample t-tes
...
test <- read.csv(file="~/Desktop/DATA/test_ear_obs.csv", header=TRUE, dec=".") #loading data
median(test$INI_ear) 
median (test$INI_obs)
...
freq_test 
```
> Interpretation: The alternative hypothesis meniotned here is stated as "true location shift is not equal to 0". Which means that the distribution of one population is shifted left/right (signifies different medians). From this information, we can see that there is a **significant difference** in peak frequency observations (p-value < 2.2e-16) but **no significant difference** in inter-note intervals (INIs) (p-value = 0.4703) between recordings made by either EARS and OBS devices. One possible interpretation from these results may be that these devices may differ in sensitivity to sound. These results suggest that while both devices capture similar temporal patterns (INI), they may differ how they record frequency related information.
...
- change color of drop down boxes to white to make them easier to see
- break up figure 3 into figure 3 a b c sections 
second set of histograms for figure 4 - make axes sizes smaller 
add info on csvs used for ## Supplemental Figure 1 
...
# My Takeaways:
Overall this code was easy to follow in order to create the figures for this paper. The ReadMe file was very well organized and I really appreciated how the research team defined all of the csv files used alongside each of the columns within the files. 
