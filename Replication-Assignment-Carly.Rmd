...
> getting an error below with the linear model i am trying to run - cross reference with supplemental figure 1 - thats what this is mirroring

## LINEAR MODEL TIME: RUNNING INTO ERROR WITH MATRIX
```{r}
INI_lm<-read.csv(file="~/Desktop/DATA/INI_ave_06_20.csv", header=TRUE, dec=".")
LM_ini<-lm(Ave_INI ~ year, data=INI_lm)
summary(LM_ini)

# Model validation figures for Figure 3 - figure supplement 1
#layout(matrix(c(1,2,3),c(3,3,3), byrow=TRUE)) #running into an error HERE!!!
par(mfrow = c(3, 3))
...
> recreating qq plots from supplemental figure as well - cross reference with supplemental section from actual paper 
- cant run them until issue with histograms is resolved - in meantime cross reference with supplemental figures themselves
...
END: COULD DO WILCOX PAIRWISE COMPARISONS !!! 
> need to load required packages when you have wifi to see what happens
- pasted the code below to review later when wifi works

"
#Supplementary material: Test OBS - EARS
#Wilcox test pariwise comparisons ==============================================
library (coin)
library(rstatix)
...
#Figure 3 - figure supplement 2:Averaged peak frequencies of the 20-Hz notes per song from all data
bout99<-read.csv(file="PF_ave_all.csv", header=TRUE, dec=".")
bout99$date <-as.Date(bout99$date, format = "%Y/%m/%d")
...
freq20_b <- ggplot(bout99, aes(x = date, y = Ave_PF, color= Region, shape= Recorder)) +
  ...
  theme(plot.title = element_text(size = 18, family = "Times", face = "bold"),
                                           ...
                                           axis.title.x = element_blank(),
                                           axis.text.x = element_text(colour="black", size = 32, vjust=0.5, angle=90),
                                           axis.text.y = element_text(colour="black", size = 32), legend.position = "top", 
                                           legend.text=element_text(size=20), legend.title=element_blank()) +
